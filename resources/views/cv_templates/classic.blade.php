...
<head>
    ...
    <style>
        ...
        .text-block-left {
            ...
            /* Triệt tiêu hoàn toàn justify lỗi */
        }
...
<body>
    ...
    <div style="color: #334155; font-size: 13px; text-align: left !important;">
        <div class="info-row" style="white-space: pre-line; margin-bottom: 5px;"><strong>Giới thiệu:</strong> <i>{{ $candidate->summary }}</i></div>
        <div class="info-row" style="white-space: pre-line; margin-bottom: 5px;"><strong>Mục tiêu:</strong> <i>{{ $candidate->objective }}</i></div>
        <div style="margin-top: 5px; font-weight: bold; color: #475569; font-size: 11px;">Tổng thời gian tích lũy kinh nghiệm thực chiến: {{ $candidate->experience_years }} năm.</div>
    </div>
</div>
            ...
            @foreach($projects as $proj)
                <div class="project-item" style="margin-bottom: 12px;">
                    <table class="w-full project-header-table" cellpadding="0" cellspacing="0">
                        <tr>
                            <td class="project-title" style="font-size: 13px; font-weight: bold; color: #0f172a;">{{ $proj['project_name'] ?? ($proj['name'] ?? 'Tên dự án') }}</td>
                            <td class="project-duration" style="font-size: 12px; font-style: italic; text-align: right; white-space: nowrap;">({{ $proj['duration'] ?? 'Chưa cập nhật' }}) tháng</td>
                        </tr>
                    </table>
                    @if(isset($proj['role']))
                        <p class="project-role" style="font-size: 13px; color: #475569; font-style: italic; margin: 2px 0;">Vị trí: {{ $proj['role'] }}</p>
                    @endif
                    @if(isset($proj['description']))
                        <p class="project-desc" style="font-size: 13px; color: #334155; white-space: pre-line; margin: 0; text-align: left !important;">{{ $proj['description'] }}</p>
                    @endif
                </div>
            @endforeach
        @else
            <p style="font-size: 13px; color: #94a3b8; font-style: italic;">Chưa có thông tin dự án.</p>
        @endif
    </div>
</div>
    ...
    <div style="color: #334155; font-size: 13px; text-align: left !important;">{!! nl2br(e($candidate->education)) !!}</div>
</div>
                    ...
                    @if($candidate->skills && count($candidate->skills) > 0)
                        <ul class="skills-list">
                            @foreach($candidate->skills as $skill)
                                <li><strong>{{ $skill->name }}</strong>@if(isset($skill->pivot->level)) <span style="color: #64748b; font-size: 10.5px;">({{ $skill->pivot->level }})</span>@endif</li>
                            @endforeach
                        </ul>
                    @else
