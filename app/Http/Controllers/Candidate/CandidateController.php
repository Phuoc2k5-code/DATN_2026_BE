...
class CandidateController extends Controller
{
  ...
  public function previewCV(Request $request)
  {
    ...
    // 1. Lấy thông tin ứng viên kèm kỹ năng của User hiện tại
    $candidate = Candidate::with(['skills', 'category'])
      ->where('user_id', $userId)
      ->first();

    if (!$candidate) {
      abort(404, 'Bạn chưa khởi tạo thông tin ứng viên.');
    }

    if (($request->wantsJson() || $request->acceptsJson()) && !$request->has('cv_template_id')) {
      return response()->json([
        'success' => true,
        'data' => [
          'id' => $candidate->id,
          'cv_template_id' => $candidate->cv_template_id ?? 1 // Lấy mẫu từ DB, nếu trống trả về 1
        ]
      ]);
    }

    // 2. Mẫu mặc định dự phòng nếu không tìm thấy mẫu nào phù hợp
    $viewName = 'modern';

    // Đọc bất chấp từ Query parameters (?cv_template_id=) hoặc JSON Body
    $requestTemplateId = $request->input('cv_template_id');

    if (!is_null($requestTemplateId) && $requestTemplateId !== '' && $requestTemplateId !== 'undefined') {
      $chosenTemplate = CvTemplate::find($requestTemplateId);
      if ($chosenTemplate && !empty($chosenTemplate->file_path)) {
        $viewName = strtolower(trim($chosenTemplate->file_path));
      }
    } else {
      // Nếu Frontend KHÔNG TRUYỀN, ta lấy mẫu từ DB của Candidate
      if (!empty($candidate->cv_template_id)) {
        $savedTemplate = CvTemplate::find($candidate->cv_template_id);
        if ($savedTemplate && !empty($savedTemplate->file_path)) {
          $viewName = strtolower(trim($savedTemplate->file_path));
        }
      }
    }

    // 4. Khớp với cấu trúc thư mục của bạn: views/cv_templates/{file_path}.blade.php
    $fullViewPath = "cv_templates." . $viewName;

    \Log::info("CV Preview Debug:", [
      'candidate_saved_template_id' => $candidate->cv_template_id ?? 'Trống',
      'request_template_id' => $requestTemplateId ?? 'Không truyền',
      'calculated_view_name' => $viewName,
      'view_file_exists' => view()->exists($fullViewPath) ? 'Có tồn tại' : 'KHÔNG TỒN TẠI (Sẽ bị ép về modern)'
    ]);

    // Kiểm tra xem file view đó có tồn tại thực tế không, nếu không thì quay về mẫu modern cho an toàn
    if (!view()->exists($fullViewPath)) {
      $fullViewPath = "cv_templates.modern";
    }

    // Trả về trực tiếp giao diện HTML cùng dữ liệu candidate
    return view($fullViewPath, compact('candidate'));
  }
